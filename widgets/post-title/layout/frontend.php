...
if ( is_page() ) {
	...
	$title_text = sprintf( '%s', get_the_date( _x( 'Y', 'yearly archives date format', 'xpro' ) ) );
} elseif ( is_month() ) {
	$title_text = sprintf( '%s', get_the_date( _x( 'F Y', 'monthly archives date format', 'xpro' ) ) );
} elseif ( is_day() ) {
	$title_text = sprintf( '%s', get_the_date( _x( '', 'daily archives date format', 'xpro' ) ) ); //phpcs:ignore WordPress.WP.I18n.NoEmptyStrings
} elseif ( is_search() ) {
