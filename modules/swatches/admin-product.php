...
class Admin_Product {
	...
	public function get_option_terms_product($taxonomy, $index) {
        ...
        <select multiple="multiple" data-placeholder="<?php esc_attr_e('Select terms', 'xpro'); ?>"
        ...
        <button class="button plus select_all_attributes"><?php esc_html_e('Select all', 'xpro'); ?></button>
        <button class="button minus select_no_attributes"><?php esc_html_e('Select none', 'xpro'); ?></button>
        <button class="button fr plus xpro_assign_new_attribute" data-type="<?php echo esc_attr( $taxonomy->attribute_type ) ?>">
			<?php esc_html_e('Add new', 'xpro'); ?>
        </button>
	...
	public function add_attribute_by_ajax() {
		...
		if(!wp_verify_nonce($nonce, 'xpro_nonce_add_attribute')) {
			wp_send_json_error(esc_html__('Request denied', 'xpro'));
		}

		if(empty($name) || empty($swatch) || empty($tax) || empty($type)) {
			wp_send_json_error(esc_html__('Insufficient data', 'xpro'));
		}

		if(!taxonomy_exists($tax)) {
			wp_send_json_error(esc_html__('Taxonomy is not exists', 'xpro'));
		}

		if(term_exists($name, $tax)) {
			wp_send_json_error(esc_html__('Term already exists', 'xpro'));
		}
		...
		wp_send_json_success(
			[
				'msg'  => esc_html__('Successfully added', 'xpro'),
				'id'   => $term->term_id,
				'slug' => $term->slug,
				'name' => $term->name,
			]
	...
	public function xpro_term_template() {
        ...
        <div id="xpro_tpl_modal" class="xpro_modal_container">
            <div class="xpro_modal">
                ...
                <div class="xpro_modal__header"><h2><?php esc_html_e('Add new term', 'xpro') ?></h2></div>
                <div class="xpro_modal__content">
                    <p class="xpro_term__name">
                        <label>
							<?php esc_html_e('Name', 'xpro') ?>
                            <input type="text" class="xpro__input" name="name">
                        </label>
                    </p>
                    <p class="xpro_term__slug">
                        <label>
							<?php esc_html_e('Slug', 'xpro') ?>
                ...
                <div class="xpro_modal__footer">
                    <button class="button button-secondary xpro_modal__close"><?php esc_html_e('Cancel', 'xpro') ?></button>
                    <button class="button button-primary xpro_add_attribute_submit"><?php esc_html_e('Add New', 'xpro') ?></button>
        ...
        <script type="text/template" id="tmpl-xpro__tpl_input__color">
            ...
            <label><?php esc_html_e('Color', 'xpro') ?></label><br>
        ...
        <script type="text/template" id="tmpl-xpro__tpl_input__image">
            ...
            <label><?php esc_html_e('Image', 'xpro') ?></label><br>
            ...
            <div style="line-height:60px;">
                ...
                <button type="button" class="xpro_upload_img_button button">
					<?php esc_html_e('Upload/Add image', 'xpro'); ?>
                </button>

                <button type="button" class="xpro_remove_img_btn button hidden">
					<?php esc_html_e('Remove image', 'xpro'); ?>
                </button>
            </div>
