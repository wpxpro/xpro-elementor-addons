...
class Xpro_Elementor_Dashboard_Ajax {
	...
	public function xpro_elementor_addons_admin_action() {
		...
		$widgets   = filter_var_array( wp_unslash( $_POST['xpro_elementor_widget_list'] ), FILTER_SANITIZE_STRING );
		$modules   = filter_var_array( wp_unslash( $_POST['xpro_elementor_module_list'] ), FILTER_SANITIZE_STRING );
		$user_data = filter_var_array( wp_unslash( $_POST['xpro_elementor_user_data'] ), FILTER_SANITIZE_STRING );
