					...
					var IDSelect2 = $( ID ).select2(
						{
							...
							ajax: {
								url: xpro_elementor_select_localize.ajaxurl + "?action=xpro_elementor_select_search_post&post_type=" + obj.data.source_type + '&source_name=' + obj.data.source_name + '&nonce=' + xpro_elementor_select_localize.nonce,
								dataType: 'json'
							},
							initSelection: function (element, callback) {
								if ( ! obj.multiple) {
									callback( {id: '', text: xpro_elementor_select_localize.search_text} );
								} else {
									callback( {id: '', text: ''} );
								}
								var ids = [];
								...
								if (ids.length > 0) {
									...
									$.ajax(
										{
											method: "POST",
											url: xpro_elementor_select_localize.ajaxurl + "?action=xpro_elementor_select_get_title",
											data: {
												post_type: obj.data.source_type,
												source_name: obj.data.source_name,
												id: ids,
												nonce: xpro_elementor_select_localize.nonce
											}
										}
									).done(
										function (response) {
											if (response.success && typeof response.data.results != 'undefined') {
												let xproSelect2Options = '';
												ids.forEach(
													function (item, index) {
														if (typeof response.data.results[item] != 'undefined') {
															const key           = item;
															const value         = response.data.results[item];
															xproSelect2Options += ` < option selected = "selected" value = "${key}" > ${value} < / option > `;
														}
													}
												)

												element.append( xproSelect2Options );
											}
							...
							IDSelect2.next().children().children().children().sortable(
								{
									...
									stop: function (event, ui) {
										ui.item.parent().children( '[title]' ).each(
											function () {
